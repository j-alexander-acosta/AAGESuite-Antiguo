...
def profesores_info(request):
    ...
    worksheet.write(0, 11, 'Fecha de Inicio Contrato')
    worksheet.write(0, 12, 'Cargo')
    worksheet.write(0, 13, 'Horas Indefinidas Actual')
    worksheet.write(0, 14, 'Horas Plazo Fijo Actual')
    worksheet.write(0, 15, 'Horas Contrato Propuestas')
    worksheet.write(0, 16, 'Horas Jornada Semanal')
    worksheet.write(0, 17, 'Asignaciones Aula Plan')
    worksheet.write(0, 18, 'Horas Aula PIE')
    worksheet.write(0, 19, 'Horas Aula SEP')
    worksheet.write(0, 20, 'Horas Aula Sostenedor')
    worksheet.write(0, 21, 'Horas disponibles')
    worksheet.write(0, 22, 'Asignación No Lectiva')
    worksheet.write(0, 23, 'Horas no lectivas disponibles')
    worksheet.write(0, 24, 'Asignación No Aula Normal')
    worksheet.write(0, 25, 'Asignación No Aula PIE')
    worksheet.write(0, 26, 'Asignación No Aula SEP')
    worksheet.write(0, 27, 'Especialidad')
    ...
    for pp in qs:
        ...
        worksheet.write(row, 11, pp.fecha_inicio)
        worksheet.write(row, 12, pp.get_cargo_display())
        worksheet.write(row, 13, pp.horas_indefinidas)
        worksheet.write(row, 14, pp.horas_plazo_fijo)
        worksheet.write(row, 15, pp.horas_semanales_total)
        worksheet.write(row, 16, pp.horas_semanales)
        worksheet.write(row, 17, pp.horas_asignadas_plan)
        worksheet.write(row, 18, pp.horas_asignadas_pie)
        worksheet.write(row, 19, pp.horas_asignadas_sep)
        worksheet.write(row, 20, pp.horas_asignadas_sostenedor)
        worksheet.write(row, 21, pp.horas_disponibles)
        worksheet.write(row, 22, pp.horas_no_lectivas_asignadas_anexo)
        worksheet.write(row, 23, pp.horas_no_lectivas_disponibles)
        worksheet.write(row, 24, pp.horas_no_aula_asignadas_ordinaria)
        worksheet.write(row, 25, pp.horas_no_aula_asignadas_pie)
        worksheet.write(row, 26, pp.horas_no_aula_asignadas_sep)
        worksheet.write(row, 27, str(pp.especialidad))
        row += 1
