...
def profesores_info(request):
    ...
    worksheet.write(0, 3, 'Fecha de Inicio Contrato')
    worksheet.write(0, 4, 'Cargo')
    worksheet.write(0, 5, 'Horas Indefinidas Actual')
    worksheet.write(0, 6, 'Horas Plazo Fijo Actual')
    worksheet.write(0, 7, 'Horas Contrato Propuestas')
    worksheet.write(0, 8, 'Horas Jornada Semanal')
    worksheet.write(0, 9, 'Asignaciones Aula Plan')
    worksheet.write(0, 10, 'Horas Aula PIE')
    worksheet.write(0, 11, 'Horas Aula SEP')
    worksheet.write(0, 12, 'Horas Aula Sostenedor')
    worksheet.write(0, 13, 'Horas disponibles')
    worksheet.write(0, 14, 'Asignación No Lectiva')
    worksheet.write(0, 15, 'Horas no lectivas disponibles')
    worksheet.write(0, 16, 'Asignación No Aula Normal')
    worksheet.write(0, 17, 'Asignación No Aula PIE')
    worksheet.write(0, 18, 'Asignación No Aula SEP')
    worksheet.write(0, 19, 'Especialidad')
    ...
    for pp in qs:
        ...
        worksheet.write(row, 7, pp.horas_semanales_total)
        worksheet.write(row, 8, pp.horas_semanales)
        worksheet.write(row, 9, pp.horas_asignadas_plan)
        worksheet.write(row, 10, pp.horas_asignadas_pie)
        worksheet.write(row, 11, pp.horas_asignadas_sep)
        worksheet.write(row, 12, pp.horas_asignadas_sostenedor)
        worksheet.write(row, 13, pp.horas_disponibles)
        worksheet.write(row, 14, pp.horas_no_lectivas_asignadas_anexo)
        worksheet.write(row, 15, pp.horas_no_lectivas_disponibles)
        worksheet.write(row, 16, pp.horas_no_aula_asignadas_ordinaria)
        worksheet.write(row, 17, pp.horas_no_aula_asignadas_pie)
        worksheet.write(row, 18, pp.horas_no_aula_asignadas_sep)
        worksheet.write(row, 19, str(pp.especialidad))
        row += 1
