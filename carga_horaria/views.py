...
def profesores_info(request):
    ...
    for pp in qs:
        ...
        worksheet.write(row, 12, pp.fecha_nacimiento)
        worksheet.write(row, 13, pp.persona.tipo_contrato)
        worksheet.write(row, 14, pp.get_cargo_display())
        worksheet.write(row, 15, pp.fecha_inicio)
        worksheet.write(row, 16, pp.horas_propuestas)
        worksheet.write(row, 17, pp.horas_sbvg)
        worksheet.write(row, 18, pp.horas_sep)
        worksheet.write(row, 19, pp.horas_pie)
